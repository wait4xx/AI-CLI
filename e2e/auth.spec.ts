...
test.describe('认证流程', () => {
  ...
  test('管理员登录并获取 token', async ({ request }) => {
    // 管理员登录
    const loginRes = await request.post('http://localhost:18333/api/auth/login', {
  ...
  test('刷新 token', async ({ request }) => {
    ...
    const refreshRes = await request.post('http://localhost:18333/api/auth/refresh', {
      data: { refreshToken },
    })
  ...
  test('无效 refresh token 返回 401', async ({ request }) => {
    const res = await request.post('http://localhost:18333/api/auth/refresh', {
  ...
  test('管理员创建用户 → 新用户登录 → 验证有效', async ({ request }) => {
    // 1. 管理员登录
    const adminLogin = await request.post('http://localhost:18333/api/auth/login', {
      data: { username: ADMIN_USER, password: ADMIN_PASS },
    })
    const { accessToken } = await adminLogin.json()

    // 2. 创建新用户
    const createRes = await request.post('http://localhost:18333/api/auth/users', {
    ...
    // 3. 新用户登录
    const userLogin = await request.post('http://localhost:18333/api/auth/login', {
      data: { username: testUser, password: testPassword },
    })
    ...
    const protectedRes = await request.get('http://localhost:18333/api/auth/users', {
  ...
  test('管理员可列出用户', async ({ request }) => {
    ...
    const usersRes = await request.get('http://localhost:18333/api/auth/users', {
      headers: { Authorization: `Bearer ${accessToken}` },
    })
  ...
  test('无 token 访问受保护路由返回 401', async ({ request }) => {
    const res = await request.get('http://localhost:18333/api/auth/users')
    expect(res.status()).toBe(401)
  })

  test('健康检查端点', async ({ request }) => {
    const res = await request.get('http://localhost:18333/health')
